...
const MethodologySection = () => (
  <section id="methodologie" className="scroll-mt-24 bg-background py-16 sm:py-20 lg:py-24">
    <div className="container mx-auto px-4">
      ...
      <div className="relative mt-12 sm:mt-14 lg:mt-16">
        ...
        <div
          className="absolute left-4 top-0 h-full w-px bg-border sm:left-5 lg:left-1/2 lg:-translate-x-1/2"
          aria-hidden="true"
        />
          ...
          {phases.map((p, i) => {
            ...
            return (
              <Reveal key={p.title} delay={i * 0.05}>
                <div className="relative grid items-center gap-4 pl-12 sm:gap-6 sm:pl-16 lg:grid-cols-[1fr_auto_1fr] lg:gap-8 lg:pl-0">
                  {/* Pastille numérotée */}
                  <span
                    className={cn(
                      "absolute left-0 top-4 grid h-9 w-9 place-items-center rounded-full text-xs font-bold text-white sm:left-0 sm:top-5 sm:h-10 sm:w-10 sm:text-sm",
                  ...
                  <article
                    className={cn(
                      "rounded-2xl border border-border bg-card p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-elevated sm:p-6",
                      p.hover,
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        <p
                          className={cn(
                            "text-[10px] font-semibold uppercase tracking-[0.15em] sm:text-xs",
                            p.tint,
                          )}
                        >
                          Phase {i + 1}
                        </p>
                        <h3 className="mt-0.5 font-display text-sm font-bold leading-snug sm:text-base sm:leading-tight sm:font-display sm:text-lg">{p.title}</h3>
                      </div>
                    </div>
                    <p className="mt-2 text-xs leading-relaxed text-muted-foreground sm:mt-3 sm:text-sm">{p.text}</p>
                  </article>
                </div>
              </Reveal>
            );
